...
**Authority:** this is the active implementation plan. `plans/local/tasks.md`, `docs/OPEN_DECISIONS.md`, versioned contracts and immutable evidence remain the completion/decision record. Superseded implementation-plan files have been removed; the retained decision IDs below preserve the still-live rules they introduced.

**Retained decision glossary (binding after superseded-plan deletion):**

- **P5-0P — existing-UI parity gate:** audit the implemented UI against the original HTML and existing matrices; every intentional composition/behavior change requires a versioned matrix amendment and regression evidence before implementation.
- **P5-1P — pre-result freeze:** before generation or result inspection, freeze reuse decisions, model/evaluation protocols, distribution/default-page expectations, exact thresholds, seeds, golden vectors and failure behavior.
- **P5-D6 / P5-D24 — cost/readiness boundary:** production deployments require attributable, decision-time-valid cost/readiness authority and may not manufacture ownership. For this PoC, §0.0 is the binding override: generated WAC is authoritative and serves margin without a client-actual gate.
- **P5-D8 — competitor evidence:** retain effective-dated match/source identity, disjoint evaluation truth, precision/recall/calibration gates, compatible currency/unit and freshness; review/rejected/stale/low-confidence matches may be visible but may not drive price action.
- **P5-D15 — demo-state coverage:** the active-rich profile must deterministically create every required visible state plus compact sparse/refusal evidence; hidden toggles, post-result rewriting and accidental one-state pages do not satisfy coverage.
- **P5-D17 — existing-page integration:** preserve approved page structure and exact metric semantics; no nearby-metric substitution, raw-ID fallback or unreviewed redesign.
- **P5-D20 — promotion uplift:** preregister the observational estimator, episodes, controls, support, uncertainty and untouched-holdout acceptance before generation; rejected/unsupported scopes produce no numeric uplift claim.
- **P5-D21 — confidence/priority/risk:** confidence is the contracted model statistic; priority/risk/driver mappings are pre-result, deterministic and tested. §2.2.1's profit-positive action/Low-Hold ordering is the approved PoC scenario specialization.
- **P5-D22 — promotion simulation confidence:** use the frozen seeded episode-block draw contract with minimum valid-draw and ≥90% numeric-result gate; Go evaluates stored accepted parameters and never refits request-time.
- **P5-D23 — Decision-#53 amendment:** **PoC-resolved (§0.0, see P-entry).** This PoC has no external approval body, so the positive Promotion branch is owner-directed engineering, not a wait-gate: build origin-safe lifecycle/mechanic evidence + the P5-D20/P5-D22 estimator/acceptance logic properly and let positive results emerge. The formal amendment + named approver is production-only and dormant here.
...
The active rich Pricing artifact still fails distribution and demo-positive acceptance. Direct artifact/API measurement shows:
...
- competitor price is populated on only 11 of 9,603 rows; Stock Cover and Forecast Demand on 866. The predecessor measurement had primary margin on zero; Workstream A has since made WAC margin live, but the current aggregate Margin Opportunity is negative (`−₹19.71L`), so presentation is available yet not retailer-positive.
...
> **The original HTML is a demo mock. Every number in it is fabricated. Its margin figures, NRV/provisions, forecast deltas, Demand-at-Risk, planner overrides, and promotion metrics are non-authoritative examples that must not be copied as data.**
...
- **"Same as original HTML" is _not_ authoritative for _values_.** We may not reproduce the mock's fabricated numbers. The remedy for thin evidence is an honest capability-owned state, never an invented value.
...
|---|---|---|---|
| **UNLOCK** | The data already exists in an accepted artifact; the cell is withheld by a *defect* or stale contract reference identified by the §2 inventory. | Wire the real value; correct the matrix row. | Real number appears. |
...
| **GOVERNED-KEEP** | The exact value is not authorized on the active branch—for example replay/workflow/privacy evidence is absent, no compatible prior version exists, or a conditional promotion/accounting gate has not passed. | **Do not fabricate.** Replace a bare missing token with a capability-specific business-reason chip, or a clearly labelled controlled scenario only where its owning contract expressly permits it. The removed Pricing `synthetic_margin_scenario` is not such a surface. | An informative, honest state; a number appears only after the owning evidence and approval pass. |
...
- **No metric substitution / relabelling.** We do not point a cell at a nearby metric. Each unlocked element satisfies its *own* frozen definition (`P5-D17`); e.g. Lost Sales Exposure is enabled only as *projected demand-at-risk* with an amended label, not by aliasing another number.
...
### A1. Historical root-cause recap — resolved by §A0
The following describes the pre-margin predecessor and is retained as historical evidence, not current implementation state. Margin was wired through every layer but structurally `null`:
...
- **Postgres:** the completed margin migration added the two numeric columns to `price_recommendations`. Any §6.0 pricing-unit/UOM/pack-conversion serving columns require a new migration, and the **complete pin set moves atomically**. **Seven are runtime guards** that fail closed (a serving 503) when the applied head differs — three Go read models (`ForecastMigrationRevision`/`forecast.go`, `InventoryMigrationRevision`/`inventory.go`, `PricingMigrationRevision`/`pricing.go`) and four Python materializers (`MIGRATION_REVISION` in `pricing/postgres.py`, `serving/postgres.py`, `scenario/postgres.py` and `inventory_publish/postgres.py`). The single `retail_intelligence_alembic_version` row cannot satisfy two constants at once, so all seven must equal the head simultaneously. **Two further clients name the head without gating on it** — the publisher's `servingMigration` (`publish/run_artifacts.py`) and the database schema expectation (`db/tests/test_forecast_schema.py`) — while `tools/build_closure_record.py` derives it. The migration-head contract test (`contracts/python/tests/test_serving_migration_pin.py`) must enumerate all seven runtime guards; before this reconciliation it checked only four (serving, publisher, Forecast, db test), so `scenario/postgres.py`, `inventory_publish/postgres.py`, `pricing/postgres.py` and the Inventory/Pricing read models could drift undetected — and `serving/scenario/inventory_publish` had in fact drifted to `0030_pricing_intents`. The content-addressed Forecast closure record and inventory entry record must then be regenerated through their governed generators (`tools/dev.py closure-record`) **against the live stack at the new head** as successor evidence; never edit those immutable records in place.
...
2. **completed:** correcting `products.reference_cost` semantics; residual work is the regression proof that no Pricing consumer treats it as store-cost authority;
...
- Price Recommendations deliberately models a **mostly well-priced catalog with bounded, profitable pockets of opportunity**. Hold-majority (by total population) is a transparent PoC scenario choice, not a claim about a retailer's future production distribution. **Amended 2026-08-14 — owner-accepted measured distribution (screen contract PDP-6.0-A2):** the pre-generation starting range (80–120 total, 3–4 Increase, 3–4 Reduce, 6–8 actions) is **superseded by the measured active-rich distribution the owner accepted as demo-positive: ~1093 recommendations (937 Hold, 146 Decrease, 10 Increase) plus ~8510 withheld assessments, Hold share ≥84%**. At least 84% of the recommendation population remains Hold; the ~156 actions are all profit-positive under the frozen `MARGIN_DILUTION` guard. The screen contract and manifest—not this prose—own the final frozen thresholds.
- The client-facing grid uses **exactly 20 rows per page**. The UI requests `limit=20`, the API reconciles `limit/offset/total`, and pagination/filtering never mounts 200 recommendation rows at once. Under the existing priority-first business sort, **page 1 shows the top 20 by priority and is action-dominant (High-priority, Decrease-dominant); well-priced Holds are Low priority and follow every High/Medium action** (amended 2026-08-14 — owner-accepted measured distribution, PDP-6.0-A2; the original 12–14 Hold / 3–4 Increase / 3–4 Reduce page-1 mix was a pre-generation target). Freeze the legitimate ordering invariant that makes this provable across seeds: actionable rows derive High/Medium priority from their positive opportunity/urgency; well-priced Hold rows derive Low priority because no price action is due. Therefore all High/Medium actions sort before Holds without an action-label sort key or post-result rearrangement. API tests assert High/Medium actions precede Holds under the default sort and that limit/offset/total reconcile. Do not relabel priority, add a screenshot-only tie-break, sample away rows, or reorder an inspected result.
...
- The recommendation cardinality (amended: ~1093 total, ~156 combined actionable per PDP-6.0-A2), exact 20-row page and positive-profit constraints are load-bearing together. Changing total cardinality, page size, priority mapping, default sort, opportunity denominator or profit guardrail requires a pre-run manifest and screen-contract amendment—not a UI-only change after generation. This amendment is that record.
- Dimension diversity is scoped to what the measured ~156 actions support (richer than the original 6–8 assumption). The default page spans at least four categories, four stores and two channels; Increase and Reduce each span at least two categories, two stores and two channels; at least three shown categories and three shown stores contain Hold plus one actionable state. A dimension containing only Hold is allowed outside the scripted default scope and is not falsely given an action merely to satisfy coverage. No category/store/channel containing actions may be monolithically Reduce when the frozen matrix requires both action directions.
...
At minimum the register specializes every state family in §2.5 and the four versioned Pricing matrices:
...
- all 14 Inventory page families listed in §2.5, including health, ageing, transfers, valuation method, expiry, replenishment, order, supplier, safety-stock, allocation, exception and stock-health states;
...
| Price Recommendations | Exact 20-row default page with 12–14 Hold, 3–4 Increase and 3–4 Reduce; positive reconciled Revenue/Margin Opportunity; complete competitor/stock/forecast and display names; recommendation/withheld, priority/risk/confidence bands, category/store/channel diversity, exact zero, detail/export/selection and pagination |
| Price Simulation | Per-unit Current/Proposed price with governed pack conversion; enabled Product/4-8-13-week/Include-Exclude/Expected-Best-Worst/Margin-Protection-Clearance choices; default Current/Proposed/AI distinction; valid/refused results, primary WAC Gross Margin, positive normal outcome and sub-floor/margin-dilution guardrails (§6.0) |
...
Supports Workstream B **ENRICH** and the §6.0 Pricing demo-positive phase. Governed by `P5-D15` (frozen demo-coverage contract in `P5-1P` *before* generation):
...
- For pricing action diversity and profit, vary genuine demand elasticity, observed support, current position inside that support, WAC, forecast demand, inventory state, competitor bounds and protection evidence. Do not insert an action target into ML or alter a recommendation after scoring. The source/config manifest records the causal cohort, while the outside verifier checks the resulting action distribution, row-level expected profit and positive aggregate opportunity.
...
- **Completed:** `ingestion/.../transforms/core.py:349-353` now emits a null product `reference_cost` rather than mislabelling selling price. Retain a regression proving no Pricing cost/margin consumer reads that product field as the WAC/store-cost ledger.
...
- **Migrations:** the margin-pct migration is complete. Add a successor migration for §6.0 pricing-unit label/base-UOM/quantity/pack-conversion serving fields and bump the **complete pin set** named in A5 in the same change — all seven runtime guards plus the publisher stamp and the db-schema test. Beware the half-truth that "scenario serving inherits the Forecast pin": the Go scenario read model *does* reuse `ForecastMigrationRevision`, but the Python `scenario/postgres.py` materializer carries its **own** `MIGRATION_REVISION` and must be bumped independently — as must `inventory_publish/postgres.py`. That misconception is exactly why both were missed. Expand the repository migration-head test to compare every runtime guard with the derived Alembic head, then regenerate the Forecast closure record and inventory entry record as immutable successor evidence via their governed generators. A partial bump must fail before startup rather than producing serving 503s. The present tree was a warning example: `serving/postgres.py`, `scenario/postgres.py`, `inventory_publish/postgres.py`, the publisher stamp and the db-schema test named `0030_pricing_intents` while the head and the four already-bumped pins (`forecast.go`, `inventory.go`, `pricing.go`, `pricing/postgres.py`) named `0031_pricing_margin_pct`. The code pins and the contract test have since been reconciled to `0031` / all seven guards in this cleanup; the two content-addressed evidence records still require regeneration against the live stack. Reconcile any such drift before introducing another migration.
...
- **Screen matrices / `P5-0P`:** every parity change (color, unlocked element, restyled withheld state, disabled-route disposition) becomes a **versioned matrix amendment + regression evidence**. The current plan is not permission for an unreviewed visual redesign.
...
### 6.0 Next implementation phase — Pricing demo-positive closure

This is the **next and exclusive implementation phase**. Complete and review all four Pricing destinations before resuming Forecast, Inventory, Data Management or other-page availability work. It is a vertical phase across contracts → Rust/Python datagen parity → ingestion → Forecast/Inventory dependencies → Pricing ML → PostgreSQL/API → UI; treating it as a front-end patch would leave the measured root causes intact.

The latest owner direction intentionally amends two earlier presentation assumptions: the Price Recommendations client page is 20 rows rather than 200, and every Price Simulation selector is enabled with a real supported choice set. Layout, labels, field order and comparison-table structure remain aligned to the original HTML; these interaction/data amendments are versioned in the existing Pricing screen contracts before implementation.

#### P-entry · Promotion branch — RESOLVED for the PoC (owner-directed; engineering, not approval)

**PoC resolution (per §0.0).** In production, result-bearing Promotion behavior requires a formal `P5-D23`/Decision-#53 amendment signed by a Product-Owner approver. **This PoC has no such governance body**, and treating that sign-off as a blocking gate is the same production-model-applied-to-a-PoC anti-pattern §0.0 exists to prevent — it is exactly what wrongly withheld margin. The owner has directed the positive Promotion branch, so there is **no approval to wait on**. What remains is real **engineering**, held to the same no-hack standard as everything else: generate origin-safe promotion plans/history with genuine lifecycle/mechanic/scope/readiness evidence, build the `P5-D20` uplift estimator and `P5-D22` acceptance/conflict logic properly, and let positive revenue/margin **emerge from that logic** — never a hardcoded flag, a faked number, or a rewritten model output. Decision #19 stays closed: cannibalisation, bundles, segment response/targeting and PII remain explicit `privacy_restricted` states.

**Production-only (dormant).** For a future real-data deployment the formal `P5-D23` amendment — named approver, evidence package, approve/refuse outcome — is reinstated as a real gate. That machinery is retained but does not gate this PoC. Consequently nothing in this phase is blocked waiting on an approval: datagen calibration, migrations, the authoritative run and Promotion implementation all proceed as ordinary engineering under §6.0.

#### P0 · Freeze Pricing contracts and measured acceptance

1. Amend the four existing Pricing parity contracts and the existing capture manifest—no parallel contract/register. Record the current `6 Increase / 855 Reduce / 5 Hold`, 11-row competitor coverage, negative Margin Opportunity, 200-row request and equal Proposed/AI result as predecessor evidence.
2. Freeze the §2.2.1 distribution/profit/page-size thresholds, `MARGIN_DILUTION` semantics, competitor coverage, pricing-unit representation, simulation vocabularies, default proposed-price rule and the rule that the normal four-page Pricing walkthrough contains no literal `Not available`, `N/A`, `null`, `undefined`, raw-ID fallback or unexplained em dash.
3. Classify every currently missing Pricing value. Populate it from accepted PoC data where it is a data element; use an explicit truthful state such as `Excluded by selection`, `Pending analyst review`, `No alert triggered`, `No conflict detected` or `Privacy restricted` only where the value is genuinely a state rather than a metric. A reason label may not conceal a missing required dataset.
4. Freeze KPI definitions: Margin Opportunity is the scoped sum of expected **incremental gross-margin improvement** for selectable accepted actions only; Hold contributes exactly zero; withheld assessments do not contribute; currency, rounding and denominator reconcile from row → API → tile. The normal scope must be strictly positive. **Honesty note:** positivity is a consequence of the newly frozen recommendation eligibility rule—margin-dilutive candidates become reason-coded withheld assessments before recommendation acceptance—not an aggregation trick that discards negative rows after they were accepted. The API must expose recommendation/withheld counts and reconciliation evidence so this distinction is reviewable.

#### P1 · Generate a retailer-positive Pricing cohort

1. Rust datagen is the sole execution authority. Add causal cohorts that produce a mostly well-priced catalog plus 3–4 profitable Increase and 3–4 profitable Reduce recommendations: elasticity, current position inside observed support, WAC, demand, inventory, competitor position and active protections must jointly support the result. Make equivalent Python code/schema/fixture changes and parity tests, but do not create a Python authoritative run.
2. Generate competitor observations at the approved market-wide or complete per-distributor scope—never `first_store` only—so every default recommendation row has admissible same-currency evidence and a display name. Generate above/equal/below positions plus Include/Exclude behavior without a missing competitor cell.
3. Build the upstream data needed by the Pricing slice even though non-Pricing **pages** are deferred: accepted 4-, 8- and 13-week Forecast evidence for every simulation-eligible SKU/store/channel; Inventory ATP/stock cover for every recommendation; receipt/ageing and clearance context for every simulation-eligible SKU; and WAC/UOM/pack evidence for unit-price conversion. Forecast and Inventory builders/verifiers/activations remain independent prerequisites and cannot be replaced with Pricing-local fixtures in the authoritative run.
4. Designate at least two simulation-demo SKUs from the actionable population (covering Increase and Reduce). The deterministic default Product is actionable and has Current, a distinct profitable manual Proposed candidate, and a distinct AI Optimal candidate; a Hold SKU may remain selectable but can never be the default used to satisfy the three-distinct-column gate.
5. Generate complete positive-default data for Price Recommendations, Price Simulation, Competitor Monitor and Promotion Planner. Promotion plans must have origin-visible lifecycle/mechanic/scope/readiness/model evidence and positive expected revenue/margin on the normal walkthrough; privacy-restricted details retain explicit privacy states rather than missing tokens.
6. Preserve all genuine exception/refusal possibilities in compact fixtures or alternate controlled selections. The default client walkthrough is positive; guardrail demonstrations remain reachable and prove the product refuses margin dilution, sub-floor price, stale competitor evidence and unsupported combinations.

#### P2 · Profit-aware Pricing build and verification

1. Resolve WAC before candidate scoring. In addition to the minimum-margin-rate floor, apply the frozen incremental-profit guard: an actionable candidate must have `expectedMarginImpactMinor > 0` and `expectedRevenueImpactMinor >= 0`. A candidate failing the profit condition is rejected with `MARGIN_DILUTION`; it cannot become an accepted Increase/Reduce recommendation.
2. Keep the current-price baseline and Hold semantics intact. Do not manufacture positive opportunity by absolute-valuing a loss, dropping negative rows from an already accepted population, changing signs, summing only convenient rows, or relabelling actions after scoring.
3. Build outside verifiers at artifact, materialized PostgreSQL, API and DOM layers for the full distribution, exact first-page mix, positive opportunity reconciliation, row-level profit rule, competitor completeness and dimension-label integrity. Any miss blocks selection/activation.

#### P3 · Price Recommendations — 20-row positive default page

1. Resolve all three current `limit: 200` call sites explicitly. `useRecommendationData` becomes the Price Recommendations grid contract: `limit=20` with normal pagination/filtering and exactly 20 recommendation rows on a full page. Price Simulation's product query is not a grid: replace its eager 200-row load with a searchable/paginated 20-option batch whose initial batch contains every 6–8 actionable SKU and whose deterministic first option is the designated demo SKU. Competitor Monitor is a separate grid and also becomes 20 rows per page with its own `offset/total` reconciliation. Auxiliary tables are not counted as recommendation rows. Loading, filter-empty and final partial-page states have separate tests.
2. Default page acceptance is exactly the contract in §2.2.1: 12–14 Hold, 3–4 Increase, 3–4 Reduce; all default rows show product/category/store/channel names, competitor price, stock cover, forecast demand, current/expected margin and reason/status/owner presentation without a bare missing token.
3. Margin Opportunity and Revenue Opportunity are strictly positive and reconcile to the visible scope. Increase and Reduce both contribute at least one profitable opportunity. Sentiment color expresses retailer benefit: positive opportunity is green; a refused loss is a warning, never a normal negative KPI.

#### P4 · Price Simulation — per-unit inputs and fully selectable scenarios

The original HTML owns the layout and order: Product, Current Price, Proposed Price, Simulation Period, Minimum Margin, Competitor Response, Demand Assumption and Inventory Objective, followed by Current / Proposed / AI Optimal comparison. The following interaction amendment is binding:

1. **Unit price, not an aggregate:** add governed `pricingUnitLabel`, base-unit quantity/UOM and canonical sellable-pack conversion. Current Price is a read-only **per base selling unit** value (for example `₹278.30 / L`); Proposed Price is edited/selected on the same per-unit basis. When the commercial SKU is a pack/drum/case, show its pack equivalent secondarily (for example `₹58,443 per 210 L drum`) and convert the proposed unit price back to the canonical sellable-SKU minor amount exactly before simulation. Never divide an aggregate revenue or period total to fabricate a unit price. These new persisted serving fields require the migration and full pin-set atomic bump (seven runtime guards + publisher stamp + db-schema test, plus successor evidence regeneration) defined in §5.
2. **Every selector enabled with supported values:** Product uses governed product/store/channel labels; Simulation Period supports `Next 4 Weeks`, `Next 8 Weeks`, and `Next 13 Weeks` only after each horizon has accepted forecast evidence; Competitor Response supports `Include` and `Exclude`; Demand Assumption supports `Expected`, `Best Case`, `Worst Case`; Inventory Objective supports `Margin Protection` and `Clearance`. Minimum Margin remains a real read-only policy input, not a fake selector. No option is enabled until its backend path, schema and test exist, and phase completion requires all listed options enabled and runnable.
3. **Proposed and AI Optimal are independent:** expose the legal candidate-price set. The initial product is the actionable demo SKU frozen in P1.4—not a Hold SKU. On product selection, initialize Proposed to a valid profitable manual alternative that differs from both Current and AI Optimal; AI Optimal remains model-derived and is never copied from Proposed. The default successful run must have three distinct prices and at least one distinct Units/Revenue/Gross Margin/Ending Stock result in each comparison column. If a user deliberately selects the AI price, equality is allowed but the UI states `Proposed matches AI Optimal`; this intentional case does not satisfy the default-walkthrough acceptance test.
4. All normal combinations return complete values and positive retailer outcomes. Unsupported/stale/sub-floor/margin-dilutive combinations return specific guardrail messages without replacing the last valid comparison with `Not available`. Competitor `Exclude` is a valid choice, not missing evidence. Clearance has accepted ageing context for the generated profile.
5. Add API and DOM parameterized tests across product × period × competitor response × demand assumption × inventory objective, plus unit/pack conversion and rounding vectors. Tests assert request changes affect the result and that Proposed/AI remain independently calculated.

#### P5 · Competitor Monitor and Promotion Planner closure

1. Competitor Monitor receives complete product/competitor display names, current comparable prices, match/availability/freshness/position/response states and positive normal recommendations. `Unknown` is permitted only as an explicit tested business state; the literal `Not available` is not.
2. Promotion Planner receives generated PoC plans/opportunities with positive default revenue and margin impact, lifecycle/calendar/list diversity and runnable owned dialogs. Resolve the existing D2 amendment before generation; if its aggregate positive branch is not approved and implemented, this Pricing phase fails rather than declaring all Pricing pages complete.
3. Sweep source, API payloads and rendered DOM for every bare missing token across all four Pricing pages, tabs, filters, details, owned modals, empty states and exports. A zero is rendered as zero; it is never treated as missing.

#### P6 · Calibration, authoritative run and exit gate

1. Use representative-scale throwaway Rust calibration seeds through ingestion → Forecast → Inventory → Pricing/Promotion. Tune only causal source cohorts before freeze; delete large calibration outputs after retaining compact measurements. The safety band must pass all seeds, including the 20-row default mix and positive profit constraints.
2. After contracts/configuration/model fingerprints are frozen, perform one authoritative Rust datagen run and dependency-ordered downstream orchestration. Apply Rust and Python source changes in sync, but run datagen with Rust only. Remove proven-unreferenced old datagen, ingestion, ML, PostgreSQL and DuckDB outputs before the run while preserving active/predecessor authority.
3. The phase exits only when all four Pricing pages pass desktop/mobile UI parity, keyboard operation, API/schema reconciliation and the scripted positive walkthrough; every selector path is runnable; no Pricing DOM contains a bare missing token; the default recommendation page contains exactly 20 rows with the approved mix; all default competitor cells are populated; Margin Opportunity is positive; and Current/Proposed/AI are distinct on the default simulation.
4. Record the final measured counts, values, identities and captures in the successor manifest. Only after this gate passes may work resume on non-Pricing pages.
...
|---|---|---|---|
| **A1 · Margin — complete for PoC** | authoritative WAC margin, min-margin floor, Tier-2 %, active PoC margin authority; production client-cost adapter remains a dormant future seam | Pricing profit cohorts and `MARGIN_DILUTION` guardrail from §6.0 | existing margin suites + positive opportunity reconciliation + WAC floor/margin-dilution tests |
...
| **A5 · Schema/contract/governance** | artifact/distribution schemas (+fingerprints), active PoC WAC-margin authority, pricing-unit/simulation fields, migration/OpenAPI fields, amendments to the four existing Pricing contracts, and formally approved P5-D23/NRV bindings | *none* (enables the run) | schema validation, decision/approval evidence and **dry-run gate** below |
...
4. Measure all reserved calibration seeds. Every seed must pass a narrower safety band inside the drafted outer thresholds, including the first 20-row page, positive revenue/margin opportunity, complete competitor cells, dimension diversity and all other state-family ceilings. This proves robustness rather than fitting one lucky seed.
...
3. **One Pricing build/lifecycle:** build → verify → materialize → prepare selection(s) → approve → activate. In this PoC the accepted WAC cost basis activates served margin as defined by §A0; the production-only external `price_margin` selection seam remains dormant. Aggregate promotion is included only on the §6.0/D2 amended and passing branch.
...
| **B4 · Pricing** | 4 (Recs / Simulation / Competitor / Promotion) | §6.0 Pricing phase: exact 20-row Hold-majority profitable page; positive reconciled Margin Opportunity; complete competitor/stock/forecast; per-unit simulation with enabled vocabularies and distinct Current/Proposed/AI; Promotion positive branch + privacy states; no bare missing token |
...
4. PoC WAC margin remains active; the Pricing phase cannot pass until the Promotion amended branch is approved, built and verified; NRV/provision branches remain separately conditional for later Inventory work;
...
7. Price Recommendations passes the 80–120 total / 6–8 actionable contract, exact 20-row default-page mix, positive revenue/margin opportunity, complete competitor cells, category/store/channel diversity and dominant-state ceilings;
...
**D2 · Promotion Planner = required positive PoC branch for §6.0 — owner-directed engineering, not an approval gate (§0.0 / P-entry).** This PoC has no external approver, so there is no formal P5-D23 sign-off to wait on; the owner has directed the positive branch. What is required is the *engineering*: origin-safe generated plans/history, real lifecycle/mechanic fields, and a genuine P5-D20 estimator + P5-D22 acceptance/conflict implementation, with positive revenue/margin **emerging** from that logic (no hardcoded flag or faked number). Pricing-phase completion requires that engineering to be built and verified, not an approval outcome. The formal amendment + named approver is production-only and dormant. Decision #19 remains closed: cannibalisation, bundles, segment response/targeting and PII stay explicit `privacy_restricted` states.

**D3 · Executive Overview (+ the six other analytics/admin routes) = governed native-disabled, out of implementation scope.** Keep them as visible, native-disabled nav entries with an accessible business reason under P5-0P, exact order/label/icon preserved. *Rationale:* implementing seven more full screens exceeds this plan's authority; a native-disabled route with a stated business reason is the governed treatment and is not a data-cell "Not available". Color-parity work still targets the 20 in-scope destinations. (If you want Executive Overview built, it is a separate scoped addition — say so and I'll add a unit.)
...
**Residual that may still show a governed (non-bare, informative) state rather than a number outside the §6.0 Pricing normal walkthrough:** replay-dependent benefits; planner workflow; privacy-restricted promotion elements; forecast deltas until two compatible accepted versions exist; disabled routes; and NRV/provisions until accounting inputs and definitions pass. Primary margin and normal Pricing competitor evidence are **not** residuals: margin is live on WAC (§A0), and §6.0 requires competitor completeness across the default Pricing experience.
