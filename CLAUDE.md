...
- Stack is UP (Docker Desktop): Postgres `127.0.0.1:5432` (db `retail_intelligence`, user `retail`), MLflow `127.0.0.1:5000`. **DB migration head `0034_expiry_waste_prior_window`** (adds nullable `prior_waste_units`/`prior_waste_minor` to `retail_serving.inventory_expiry_waste` for the Waste Reduction prior-period comparison). Chain: `0032_pricing_unit_fields` (per-unit Price Simulation) → `0033_promotion_positive_branch` → `0034`.
