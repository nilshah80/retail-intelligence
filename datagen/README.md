...
Source spec v13 supports the locale-pack range `2005-01-01` through `2026-12-31`: up to 22
...
The v12 PoC return policy deliberately publishes every processed return as `NO_RESTOCK`; no
...
The v12 demand process uses a cross-year, mean-reverting regular-price path instead of a repeating
...
**Config-driven screen-completeness evidence implemented by source spec v12:**
...
single DuckDB mirror is the complete v13 publication matrix; JSONL remains an ingestion adapter
...
## Generator version and what moves a run id

`GENERATOR_VERSION` in `src/retail_datagen/__init__.py` is part of the run id, so a change to what
the generator *emits* has to move it. Otherwise the output directory already exists,
`writer.reused` short-circuits, and a regeneration silently returns the old run — the failure looks
exactly like a successful no-op.

| Version | What changed in the emitted source |
| --- | --- |
| `0.15.0` | `store_stockout_events` written for the first time (computed and discarded since v13); store transfer receipts carry `unitCostMinor` in minor units rather than the major-unit `_baseCost`, which had every store cost a hundredfold small. |
| `0.16.0` | Replenishment lane transit varies **per lane** instead of taking one of two run-wide policy constants, and the vendor master stamps `observedAt`. |

**Why 0.16.0.** `primaryLaneTransitDays` / `spillLaneTransitDays` are single policy values, so every
rank-1 lane in the network carried an identical transit time. Every downstream replenishment
recommendation therefore resolved the same lead time — 2 days on all 720 orders — and the planner's
Lead Time and Expected Receipt columns were one value repeated down the page. The spread is now
deterministic in the lane's own identity and *additive* on the declared floor, so the policy still
sets the minimum and a regeneration reproduces the same lanes exactly.

The vendor master previously carried no timestamp at all, which left the staging layer with no
honest `known_as_of` to gate on — so the dimension could not be landed, and every screen that names
a supplier printed its UUID.

**Regenerating is a governed act.** Decision #89: a regeneration moves `sourceSnapshotId`, because
the id hashes Parquet bytes, and every fingerprint derived from it moves with it. The ML stages fail
closed against `contracts/ml/expected-pin.json` until the pin is re-established with equivalence
evidence. `tools/dev.py datagen` refuses without `--regenerate` for that reason, and reports the
cost (~90 minutes for the ten-year demo) rather than starting silently.

**Output lives under `datagen/output/<scenario>/run-<id>/`**, not under a directory called `data`.
A "clean everything" that only clears `ingestion/data` and `ml/data` leaves the generated source in
place — two complete runs of the ten-year demo are ~30 GB.

## Implemented source-spec v12
...
- native effective-dated `storeAssortment.observedAt` evidence. Downstream ingestion can combine
  that source observation with business-day close to make a materialized zero sale origin-safe;
  v11 inputs without this field remain explicitly landing-backfilled and non-PIT;
...
warehouse/fulfillment nodes across India and the US, carries 1,440 sellable SKUs across 10
...
The currently accepted Phase-2 and ML input is the v0.16.0/v13 run generated by the command above:

```text
run id:          run-adac9e85dccb56e8
config hash:     6dd93041e093e7d6294dd5925a031146c447ce0159169ad321a05a231e77b094
source dates:    2016-07-28 through 2026-07-28
local path:      datagen/output/multi-market-10-year-demo/run-adac9e85dccb56e8
```

It is landed immutably and published through Phase 2; see the repository-root `README.md` for the
complete generation → ingestion → API → UI runbook.

Acceptance evidence:

```text
manifest hash:   f9fc6c4a6f3628c4ee43e3b498915200a15edb8392f5e23abe2a363b6936fd73
objects:         9,938
source rows:     255,061,144
run size:        16 GiB promoted, ~54 GiB staging peak
generation:      4,783.85 s under performance
status:          Gate A pass; Gate B pass; exact reconciliation; frozen ML input pin
```

The object and row counts grew with source spec v13; every v12 control held. The per-currency
gross/net/tax/unit totals, the 0.976855 fill rate, the 8,800,322 orders and the 15,785,727 order
lines are unchanged across four regenerations and the spec bump, which is what says no existing row
moved — `ingestion/tests/test_pinned_run.py` asserts each of them.

The larger assortment is an evidence rebaseline, not gate tuning. Native assortment observation
fixes the historical zero-label provenance defect; the already-frozen forecast acceptance gates
are evaluated downstream and have not yet accepted a forecast version.
...
It is not the Phase-2 pin after the v0.12.0/v11 correctness and realism corrections. Do not mix
its measurements or artifacts with the accepted run above.
...
and forecasting-realism corrections. The accepted v11 run named above is a fresh generation, not
an in-place repair or rename of either historical run.
...
historical comparison evidence. The accepted Phase-2 input is the separately generated
v0.16.0/v13 run named above; changing only an execution profile does not change its scenario
config hash or logical source contract.
